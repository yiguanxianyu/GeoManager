...
- 新版管理后台位于 `/admin/`，前端基于 Ant Design Pro 组件实现，优先提供操作日志、系统设置、认证授权三类系统管理能力。
...
| 功能模块 | 说明 |
|----------|------|
| 操作日志 | 新版后台提供日志查询、筛选、导出静态界面；旧版后台保留模型只读查看能力 |
| 系统设置 | 新版后台提供基础配置表单和参数管理静态界面；旧版后台保留 `SystemSetting` 模型管理 |
| 认证授权 | 新版后台提供用户管理和角色权限配置静态界面；旧版后台保留 Django User/Group/Permission 管理 |
