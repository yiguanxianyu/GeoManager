...
用户最终生效的功能权限由用户组权限和单用户直授权限合并得到，再扣除用户在个人设置中主动关闭的权限。后台用户列表返回 `directPermissions` 表示单独授予该用户的功能权限，`effectivePermissions` 表示最终生效权限；具备 `core.manage_auth` 和 `core.manage_feature_permissions` 的管理员可通过 `POST /api/users/{userId}/permissions/` 写入 `directPermissions`，该操作不会修改用户所属用户组或用户组权限配置。
...
- 后台认证授权入口由 `core.manage_auth` 控制；“新建用户”还需要 `core.create_user`，创建用户组和配置用户组/单用户功能权限还需要 `core.manage_feature_permissions`。仅具备认证授权但不具备新建用户权限的账号不能调用 `POST /api/users/` 创建用户；调用 `POST /api/users/` 创建用户时 `groupIds` 必填且至少包含一个非超级管理员用户组。启用/停用用户通过 `POST /api/users/{userId}/` 写入 `isActive`，删除用户通过同一接口提交 `action=delete` 完成；用户组成员通过 `POST /api/users/{userId}/groups/` 更新，单用户功能权限和指定日志用户组通过 `POST /api/users/{userId}/permissions/` 写入 `directPermissions`、`operationLogGroupIds`，用户组通过 `POST /api/groups/{groupId}/` 更新或提交 `action=delete` 删除；当前登录用户和初始化管理员不能被停用或删除。重置密码通过 `POST /api/users/{userId}/password/reset/` 生成随机密码，密码规则与后台创建用户一致，`generatedPassword` 仅在响应中返回一次，当前登录用户不能重置自己的密码。
- Dashboard 入口由 `core.access_admin` 控制；数据资源、图层数、栅格数量、用户数量和活跃用户卡片分别由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制，接口只返回已授权内容。活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
