...
- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选当前用户可见或本人上传的存量数据，返回当前主体可见的数据访问角色、上传用户、数据大小、条目数和 `canManageAccess`。超级管理员可查看全部资源；仅具备 `catalog.add_dataresource` 时只返回当前用户上传的数据资源。
