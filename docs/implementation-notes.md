...
- 天地图 Provider 只解析最多 4 KiB 的错误正文并仅保留安全业务码；`301007`、`301018` 和强语义凭证提示按凭证错误立即失败，诊断对象与提示不得保存瓦片 URL、Key 或原始响应正文。无法确认原因的单次 HTTP 403 按瞬时错误处理，不再直接等同于整张底图故障。
- 未分类 403 最多使用一个备用节点重试；网络异常、408、425 和 5xx 最多使用两个备用节点重试；429 继续遵守 `Retry-After`、固定在原节点并作用于共享请求队列。所有实际请求继续复用原有页面级并发与启动间隔限制，不另建绕过限速的请求通道。
