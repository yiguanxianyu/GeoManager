...
- 平面缩略图使用 Web Mercator 2D OSM 瓦片按当前 bounds 平铺，瓦片级别使用主图缩放向下取整后小 2 级，不按具体瓦片数量回退缩放级别。
- 缩略图 SVG viewBox 以当前范围中心构造，范围指示器使用红色细框，中心点始终与当前范围中心对齐。
- 缩略图只展示底图瓦片、当前范围框和中心点，不承载栅格符号化；栅格渲染仍由后端瓦片/PNG 服务负责。
